...
class StringUtil {
     ...
     * キャピタル文字列
     */
    static formatFromSnakeToCamel(value) {
        const regex = /_[a-z]/g;
        let capital = value.replace(regex, function (matchChar) {
     ...
     * スネーク文字列
     */
    static formatFromCamelToSnake(value) {
        const regex = /[A-Z]/g;
        // 先頭が大文字の場合は小文字に変換
        if (value.length > 0 && value[0] === value[0].toUpperCase()) {
            value = value[0].toLowerCase() + value.slice(1);
        }
        return value.replace(regex, function (matchChar) {
