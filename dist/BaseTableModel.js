    ...
    executeUpdateId(id, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const [key, value] of Object.keys(options)) {
                if (key in this.Columns === false) {
                    throw new Error(`${key}は${this.TableName}に存在しません。`);
                }
                if (value === undefined) {
            ...
            vars.push(ToValueUtil_1.default.toValue(this.Columns['id'].type, id));
            const sql = `UPDATE ${this.TableName} SET ${updateAlias.join(',')} WHERE id = $${vars.length}`;
